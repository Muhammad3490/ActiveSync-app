...
submit2.addEventListener("click", (event) => {
  ...
  if (userGoal === "Weight gain") {
    protien = userWeight * 2.2;
  } else if (userGoal === "Weight loss") {
    protien = userWeight * 1.8;
  } else {
    protien = userWeight * 1.6;
  }
...
function generateExerciseInputs() {
  ...
  for (let i = 1; i <= numExercises; i++) {
    ...
    exerciseLabel.classList.add("m-m", "text-small", "block");
    exerciseLabel.textContent = `Ex ${i}:`;
...
submit3.addEventListener("click", () => {
  ...
  delContainer.classList.add("delete-btn-container", "flex-right", "m-m");
  ...
  delBtn.addEventListener("click", () => {
    // Ask the user for confirmation before deleting the workout card
    const confirmDelete = confirm(
      "Are you sure you want to delete this workout?"
    );

    if (confirmDelete) {
      // Remove the corresponding workout card
      workoutContainer.removeChild(workoutCard);
      displayList.style.display = "none"; // Hide the exercise list if it's open
    }
  });
  ...
  workoutCard.appendChild(delContainer);
...
function countSteps(event) {
  ...
  if (
    totalAcceleration > accelerationThreshold &&
